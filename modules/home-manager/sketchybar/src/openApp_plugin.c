...
void handler(env env) {
    ...
    bool is_aerospace = (strcmp(sender, "aerospace_custom_app_switched") == 0);
    bool is_front_app = (strcmp(sender, "front_app_switched") == 0);

    if (!is_aerospace && !is_front_app) { return; }

    if (is_front_app) {
        if (strcasecmp(info, "puppy") != 0) return;
    } else if (is_aerospace) {
    ...
    if (strcasecmp(info, "puppy") == 0) { info = "puppy"; }
