...
if [ "$1" = "$FOCUSED_WORKSPACE" ]; then
    ...
    sketchybar --set $NAME label.drawing=on background.drawing=on label.color=$WHITE icon.color=$WHITE
elif [ -n "$HAS_WINDOWS" ]; then
    # Case 2: It's NOT focused, but it HAS windows. Show it (but no background highlight).
    sketchybar --set $NAME label.drawing=on background.drawing=off label.color=$GRAY icon.color=$GRAY
else
