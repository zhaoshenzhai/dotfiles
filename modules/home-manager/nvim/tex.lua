...
vim.keymap.set('n', '<C-S-d>', '<cmd>write<CR><cmd>!rm -f *.aux(N) *.bbl(N) *.bcf(N) *.bcf-SAVE-ERROR(N) *.bbl-SAVE-ERROR(N) *.blg(N) *.fdb_latexmk(N) *.fls(N) *.log(N) *.run.xml(N) *.synctex.gz(N) *.synctex\\(busy\\)(N)<CR><CR>', opts)
