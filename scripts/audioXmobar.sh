...
if [ "$lines" -eq 2 ]; then
    ...
    if [ "$mute" = true ]; then
        printf "%s %s" "<fn=2></fn><hspace=15/>" "Muted"
    else
        speakerVol=`pamixer --sink "$speaker" --get-volume-human`
        printf "%s %s" "<fn=2></fn><hspace=15/>" $speakerVol
    fi
    ...
    if [ "$mute" = true ]; then
        printf "%s %s" "<fn=2>󰋋</fn><hspace=15/>" "Muted"
    else
        headphoneVol=`pamixer --sink "$headphone" --get-volume-human`
        printf "%s %s" "<fn=2>󰋋</fn><hspace=15/>" $headphoneVol
    fi
fi
